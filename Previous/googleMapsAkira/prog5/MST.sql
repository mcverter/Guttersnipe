  ...
  BEGIN
	...
	while (endIndex <> 0) do 
		...
		insert into ForestTable VALUES (pointInt, pointInt);
...
BEGIN
	...
	SELECT LENGTH(points) - LENGTH(REPLACE(points, ',', '')) INTO NumElements;
	...
	WHILE (ElementCounter <= (NumElements+1)) DO
	      ...
	      IF (STRCMP(Element, Eliminated) != 0) THEN
...
SET @pointList = '8,27,55,75,96,110,120,131,154,179,185,192,222,236,249,251,277,287,296,298,299,360,366,385,389,390,420,422,428,431,454,494,495'; CALL LimitedKruskal(@pointList, 13, @length); SELECT @length as 'length', @pointList as 'pointList' ;
