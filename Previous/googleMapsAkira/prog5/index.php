...
$startLocation="0";
$endLocation="0";
if ((isset($_POST["LeastPath"]))
    && (isset($_POST["startPoint"])) 
    && (isset($_POST["endPoint"]))
    && (($_POST["startPoint"])>0) 
    && (($_POST["endPoint"])>0)
    && (($_POST["endPoint"])!=($_POST["startPoint"]))
    )
  {
    $leastPath="Y";
    $startLocation=  $_POST['startPoint'];
    $endLocation=  $_POST['endPoint'];
    debugPrint("lEAST PATH IS YES.  Start locaiton is $startLocation.  End location is $endLocation\n");
  }

$processDistance="N";
if (isset($_POST["Distance"]))
  {
    debugPrint("In index.  distance is set");
    $processDistance="Y";
  }

$distanceAllowance=25;

if (isset($_POST["distanceAllowance"]))
  {
    $distanceAllowance=$_POST["distanceAllowance"];
  }
...
$downloadURLArgs = "R=" . $rootius . "&T=" . $allTypes . "&F=" . $allRefines . "&O=" . $circ 
  . "&Z=" . $processRefines  . "&B=" . $processBudget . "&A=" . $allowance  
  . "&D=" . $processDistance . "&M=" . $distanceAllowance 
  . "&L=" . $leastPath. "&S=" . $startLocation . "&E=" . $endLocation;
...
?>
 ...
 <script> window.dhx_globalImgPath = "anotherSlider/dhtmlxSlider/codebase/imgs/"; </script>
	<script  src="anotherSlider/dhtmlxSlider/codebase/dhtmlxcommon.js"></script>
	<script  src="anotherSlider/dhtmlxSlider/codebase/dhtmlxslider.js"></script>
	<script  src="anotherSlider/dhtmlxSlider/codebase/ext/dhtmlxslider_start.js"></script>
	<link rel="STYLESHEET" type="text/css" href="anotherSlider/dhtmlxSlider/codebase/dhtmlxslider.css">
	...
	var djikstra = "<?php echo $leastPath ?>";
	...
	geocoder.geocode( 
		...
		downloadUrl("generateMarkers.php?" + GET_arg, function(data) 
				...
				refinementHTML = "<table border=\"1\" >\n\t<tr> <b> <td> select </td> <td> id </td> <td> name </td> <td> expense </td><td>start</td><td>end</td></b></tr>\n";    
				mstLen = 0;
				totalExpense = 0;
				...
				for (var i = 0; i < markers.length; i++) 
				{
				    ...
				    var expense = markers[i].getAttribute("expense");
				    var expenseNum = expense*1;
				    ...
				    rowData += expense;
				    rowData += " </td> ";
				    rowData += " <td><input type=\"radio\" name=\"startPoint\" value=";
				    rowData += id;
				    rowData += " </td> ";
				    rowData += " <td><input type=\"radio\" name=\"endPoint\" value=";
				    rowData += id;
				    rowData += " </td> ";
				    rowData += " </tr> \n";
				...
				var MSTNode = xml.documentElement.getElementsByTagName("MST");				   
				if (MSTNode && MSTNode[0])
				  {
				    var mstLen = MSTNode[0].getAttribute("MSTlen");
				    if (!mstLen)
				      {
					mstLen = 0;
				      }
				...
				refinementHTML += "<p><h3> Total Expense : "   + totalExpense + "</h3></p>";
				
				refinementHTML += "<p><h3> Distance : "   +  mstLen + "</h3></p>";
...
<input id="allowance" name="allowance" type="text"  /> </label>


<script>
//slider 100-300, step 1;
var allowanceSlider = new dhtmlxSlider(null, 500, 'simplegray', false, 0, 500, <?php echo $allowance ?>, 1);
allowanceSlider.linkTo('allowance');
allowanceSlider.init();
</script>

    <p>
      <input type="submit" name="Budget" value="Budget" />
    </p>
...
<hr>
<br><br>
<h1> Choose a Distance Allowance:</h1>
<br> 
<label> DISTANCE ALLOWANCE: 
<input id="distanceAllowance" name="distanceAllowance" type="text"  /> </label>


<script>
//slider 100-300, step 1;
var distanceAllowanceSlider = new dhtmlxSlider(null, 500, 'simplegray', false, 0, 250, <?php echo $distanceAllowance ?>, 1);
distanceAllowanceSlider.linkTo('distanceAllowance');
distanceAllowanceSlider.init();
</script>
...
<br>
<p>
      <input type="submit" name="LeastPath" value="LeastPath" />
</p>
</div>
