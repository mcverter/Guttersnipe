...
//*********************************
//
// PROCESS THE LeastPath Argument
//
//*********************************
$dijkstra="N";
...
if (isset($_GET["L"])
    ...
    )
  {
    ...
    $startPoint=$_GET["S"]; 
    $endPoint=$_GET["E"]; 
    debugPrint("Dijkstra is " . $dijkstra);
    if ($dijkstra=='Y')
      {
	
	$dijkstraQuery = " CALL  ReducedDijkstra ('$itemList', $startPoint, $endPoint) ";
	
    debugPrint("Dijkstra Query is " . $dijkstraQuery);
...
if (!$db_selected) {
  die ('Can\'t use db : ' . mysql_error());
	...
	$result = mysql_query($dijkstraQuery);
	if (!$result ) 
	  {  
	    debugPrint ('Invalid query: ' . mysql_error());
	    die('Invalid query: ' . mysql_error());
	  } 	

	$dijkstraConstraint = " ( id=0 ";
	while ($row = @mysql_fetch_assoc($result))
	    {  

	      //	      $dijkstraConstraint="(id =" ;
	      $v1 = $row['v1'];  
	      $v2 = $row['v2'];  

	      $dijkstraConstraint .=  "  or id=$v2 or id=$v1 "; 
	      debugPrint("Our new poinst are $v2 and $v2\n");
	    }
	$dijkstraConstraint .= " ) ";
      }
...
$query;
 if ($doAllowance == 'Y')
 {
   ...
   {
	$query = "SELECT * FROM SampleData WHERE $dijkstraConstraint  ";
   }
...
debugPrint ("\ntheee querrrry issssss QUERY: $query\n");
...
while ($row = $result->fetch_assoc())
  {  
    ...
    if ($dijkstra=='Y')
      {
	$lineNode = $dom->createElement("linePoint");
	$newLineNode = $parnode->appendChild($lineNode);
	
	$newLineNode->setAttribute("lat", $row['lat']);  
	$newLineNode->setAttribute("lng", $row['lng']);  
