...
const BaseController = require('./BaseController');
  ...
  selectOneRecord(id) {
    return knex(TABLES.SHAREABLES)
      ...
      .join(TABLES.THING, `${TABLES.SHAREABLES}.thing_id`, `${TABLES.THING}.id`)
      .join(TABLES.TYPES, `${TABLES.THING}.type_id`, `${TABLES.TYPES}.id`)
...
*/
    ...
    createDBTable(knex, Promise) {
      ...
      then(()=>{
        return Promise.all([
          ...
          knex.schema.createTable(TABLES.THING, table=>{
            table.increments('id').primary();
    ...
    dropDBTable(knex, Promise) {
      return Promise
        ...
        .then(()=>
          Promise.all([
            knex.schema.dropTable(TABLES.THING),
