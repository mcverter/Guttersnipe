...
(function (angular, app, _) {
  ...
  .controller('HierarchicalSelectorGroupCtrl', ['$log', '$debug', '$scope', '$element',
    function ($log, $debug, $scope, $element) {
      ...
      var selectedMetric, groupingLevel, filters = {};
      ...
      $scope.registerComponent({
        saveValues: function(args){
          if ($scope.groupingLevel)
            args.groupingLevel = $scope.groupingLevel;
          
          if ($scope.selectedMetric)
            args.selectedMetric = $scope.selectedMetric;
            
          if (view.state && view.state.filter) {
            for (var i in view.state.filter) {
              if (view.state.filter[i]) {
                args['filter_' + i] = view.state.filter[i];
              }
            }
          }
