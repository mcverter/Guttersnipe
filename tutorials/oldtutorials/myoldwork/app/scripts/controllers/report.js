(function (angular, app, _, URI) {
  ...
  app.controller('ReportCtrl', ['$scope', '$log', '$report', '$routeParams', '$favorite', '$viewingHistory', '$location', '$config', '$googleVisualization', '$api',
    function ($scope, $log, $report, $routeParams, $favorite, $viewingHistory, $location, $config, $googleVisualization, $api) {
      var categoryId = $routeParams.categoryId,
          ...
          updateTable = function updateTable(parameters) {
            ...
            if (!parameters || (jasperView() && !$scope.viewModeEnabled('table')))
              return;
            ...
            report.table(parameters).then(function (t) {
              return $googleVisualization.then(function (vis) {
          ...
          controlPromise.then(function(c) {
            ...
            _.each(c, function (item) {
              if (_(item.value).isUndefined())
                resetControl(item);
              if (report.filters && !report.filters[item.id])
